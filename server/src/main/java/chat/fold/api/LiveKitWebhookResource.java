...
import chat.fold.db.VoiceModerationRepository;
    ...
    public Response receive(WebhookEvent event, @Context RoutingContext rc, @Context HttpHeaders headers) {
        ...
        try {
            if (liveKitService.isManaged()) {
                // Managed mode: validate X-Fold-Central-Secret from central service
                String centralSecret = headers.getHeaderString("X-Fold-Central-Secret");
                ...
                if (centralSecret == null || centralSecret.isBlank() || expected == null || !expected.equals(centralSecret)) {
                    LOG.warnf("Managed webhook rejected: invalid or missing X-Fold-Central-Secret");
