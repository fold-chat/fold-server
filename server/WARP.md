...
## Config
- `@ConfigMapping` interfaces in `chat.fold.config`, prefix `fold.<domain>`.
- Every property → `FOLD_*` env var in `application.properties`.
- `@WithDefault` for defaults, `Optional<String>` for optional.

### Runtime Config
- `RuntimeConfigService` (`chat.fold.config`) — CDI bean, in-memory `ConcurrentHashMap` cache backed by `server_config` table.
- Use for admin-overridable keys (e.g. `fold.livekit.mode`, `fold.livekit.max-participants`). Use `@ConfigMapping` for bootstrap/static config.
...
- Voice mode switching: `fold.livekit.mode` overridable at runtime (off/embedded/external/managed). `LiveKitService.getMode()` reads runtime first, falls back to `@ConfigMapping`.

## API Resources
- `chat.fold.api`, `@Path("/api/v0/<entity>")`, JSON in/out.
- DTOs: `record` types nested in resource class.
- Errors: `Map.of("error", "<code>", "message", "<text>")`.
- Auth: `sc()` helper returns `FoldSecurityContext` → `getUserId()`, `getUsername()`.
- CRUD: 201 create, 200 update, 204 delete, 404 not found.

## Database
- `DatabaseService`: `execute()` (write), `query()` (read), `transaction()`.
- Repos in `chat.fold.db`, `@ApplicationScoped`. Return `Optional<Map<String, Object>>` / `List<Map<String, Object>>`.
